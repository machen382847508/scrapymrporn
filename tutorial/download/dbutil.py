...
class DButils():
    ...
    _cursor = None
    def __init__(self):
        self.db = MySQLdb.connect(host='127.0.0.1', user='root', passwd='x5', db='mrporn', charset='utf8')
    ...
    def update_finish_download(self,pornid):
        sql = "UPDATE PORNS SET PORNDOWNLOAD = 'T',DOWNTIME=NOW() WHERE PORNDOWNLOAD = 'R' AND PORNID = " + str(pornid)
        self._cursor.execute(sql)
        self.db.commit()
        self.db.close()
